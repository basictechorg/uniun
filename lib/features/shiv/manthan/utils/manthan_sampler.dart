// lib/features/shiv/manthan/utils/manthan_sampler.dart
import 'dart:convert';
import 'dart:math';
...
class ManthanCombo {
  ...
  const ManthanCombo(this.noteIds, this.signature);
}

/// SHA-256 of the sorted, '|'-joined note ids. Order-independent so
/// {A,B} and {B,A} collide.
String manthanSignature(List<String> noteIds) {
...
/// space is exhausted.
List<ManthanCombo> sampleUnseenCombos({
  ...
  while (out.length < count && seen.length < space) {
    ...
    for (var attempt = 0; attempt < maxAttemptsPerCombo; attempt++) {
      ...
      final sig = manthanSignature(picked);
      if (seen.contains(sig)) continue;
      seen.add(sig);
      found = ManthanCombo(picked, sig);
      break;
    }
