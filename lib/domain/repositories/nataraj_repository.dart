// lib/domain/repositories/nataraj_repository.dart
...
abstract class NatarajRepository {
  /// Oldest `buffered` card for the scope, or null when the buffer is empty.
  Future<Either<Failure, NatarajCardEntity?>> nextBufferedCard(String scopeId);
  ...
  Future<Either<Failure, Unit>> insertBufferedCards(List<NatarajCardEntity> cards);

  /// Set a card's status and stamp lastSeenAt = now.
  Future<Either<Failure, Unit>> updateStatus(
      String scopeId, String signature, NatarajCardStatus status);

  Future<Either<Failure, int>> countByStatus(
      String scopeId, NatarajCardStatus status);
