...
//
//  VENDORED, verbatim, from receive_sharing_intent 1.8.1
//  (ios/Classes/RSIShareViewController.swift) with two extra safety imports
//  (AVFoundation, UniformTypeIdentifiers) added explicitly.
//
//  Why vendored instead of podding the plugin into this extension:
//  receive_sharing_intent's ObjC umbrella (`ReceiveSharingIntentPlugin.h`)
//  `#import <Flutter/Flutter.h>`. This app-extension target is NOT a Flutter
//  target and can't resolve the Flutter engine framework (delivered to Runner
//  via SwiftPM / Generated.xcconfig, which an extension doesn't include), so
//  linking the pod here fails to build the module. RSIShareViewController and
//  its data types (SharedMedia.swift) use only UIKit/Foundation/AVFoundation,
//  so we compile them directly into the extension and keep the pod on Runner
//  only (for the Dart plugin side). Duplicate symbols are fine — the .appex is
//  a separate Mach-O from Runner.app. Keep in sync if the plugin is upgraded.
//
