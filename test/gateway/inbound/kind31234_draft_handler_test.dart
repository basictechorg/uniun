...
void main() {
  ...
  Future<Map<String, dynamic>> buildWrap({
    ...
    final encrypted = overrideOuterContent ??
  ...
  test('decrypts a wrap from self and writes a fresh DraftModel row', () async {
    ...
    final row = await isar.draftModels.filter().draftIdEqualTo('draft-1').findFirst();
  ...
  test('empty outer content = NIP-37 deletion signal → local row removed', () async {
    await handler.handle(
      await buildWrap(
        draftId: 'd-1',
        author: me,
  ...
  test('parses published-as tag into publishedAsEventId (tombstone propagation)', () async {
    await handler.handle(
      await buildWrap(
        draftId: 'd-1',
        author: me,
  ...
  test('cross-device sweep: published-as triggers local UUID→eventId rewrite', () async {
  ...
  test('reply marker lands in replyToEventId; mention stays in eTagRefs only', () async {
    await handler.handle(
      await buildWrap(
        draftId: 'r-1',
        author: me,
    ...
    final row =
        (await isar.draftModels.filter().draftIdEqualTo('r-1').findFirst())!;
    expect(row.rootEventId, 'root-id');
  ...
  test('malformed (non-decryptable) content is dropped silently — no row written', () async {
  ...
  test('preserves original createdAt across resyncs', () async {
    ...
    final firstCreated =
        (await isar.draftModels.filter().draftIdEqualTo('d-1').findFirst())!.createdAt;
    ...
    final row =
        (await isar.draftModels.filter().draftIdEqualTo('d-1').findFirst())!;
    expect(row.createdAt, firstCreated, reason: 'createdAt frozen on first write');
