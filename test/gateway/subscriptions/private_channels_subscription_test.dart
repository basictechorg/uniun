...
void main() {
  ...
  test('main filter caps application messages (9023) to last 7 days', () async {
    await isar.writeTxn(
        () => isar.privateChannelModels.put(privateChannelSeed('grp1')));
    final filter = await PrivateChannelsSubscription()
  ...
  test('private-channel messages honor a custom sync window', () async {
    await isar.writeTxn(
        () => isar.privateChannelModels.put(privateChannelSeed('grp1')));
    final filter = await PrivateChannelsSubscription().buildFilter(
        SubscriptionContext(
  ...
  test('MLS control plane + metadata companions are uncapped', () async {
    await isar.writeTxn(
        () => isar.privateChannelModels.put(privateChannelSeed('grp1')));
    final companions = await PrivateChannelsSubscription()
    ...
    expect(meta.subId, 'private_channels_meta');
    ...
    expect(mls.subId, 'private_channels_mls');
