...
void main() {
  ...
  test('main filter caps messages (kind 42 only) to last 7 days', () async {
    await isar.writeTxn(() => isar.groupModels.put(groupSeed('chan1')));
    final filter =
        await GroupsSubscription().buildFilter(SubscriptionContext(isar: isar));
  ...
  test('group main filter honors a custom sync window', () async {
    await isar.writeTxn(() => isar.groupModels.put(groupSeed('chan1')));
    final filter = await GroupsSubscription().buildFilter(SubscriptionContext(
  ...
  test('companions fetch creation + metadata uncapped', () async {
    await isar.writeTxn(() => isar.groupModels.put(groupSeed('chan1')));
    final companions = await GroupsSubscription()
